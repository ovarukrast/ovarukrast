...
const SerVsEstar: React.FC<SerVsEstarProps> = ({ exercise, onFinish }) => {
    ...
    return (
        <div className="bg-white p-6 sm:p-8 rounded-lg shadow-lg">
            ...
            <p className="text-slate-600 mb-8">Elige la opción correcta para cada frase.</p>
            ...
            <div className="space-y-8">
                {exercise.questions.map((q, index) => {
                    ...
                    return (
                        <div key={index}>
                            <p className="text-lg text-slate-700 mb-4">{index + 1}. {q.sentence.replace('___', <span className="font-bold text-blue-600">___</span>)}</p>
                            ...
                            {submitted && (
                                <div className={`mt-4 p-4 rounded-md text-sm ${isCorrect ? 'bg-green-50 text-green-800' : 'bg-red-50 text-red-800'}`}>
                                    <div className="flex items-start">
                                        {isCorrect ? <CheckCircleIcon className="h-5 w-5 mr-2 text-green-500" /> : <XCircleIcon className="h-5 w-5 mr-2 text-red-500" />}
                                        <div>
